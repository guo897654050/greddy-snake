...
const varWidth = `${width * 20}px`
const varHeight = `${height * 20}px`
</script>
...
<template>
  <div style="text-align: center">
    <h1>得分：{{ game.count }}</h1>
    <div class="wrapper">
      <div :key="index" v-for="(_, index) in map"
        style="width: 20px; border: 1px solid #ccc; box-sizing: border-box"></div>
      <div :key="index" v-for="(item, index) in g.body" :style="{
        position: 'absolute',
        width: '20px',
        height: '20px',
        left: `${item.x * 20}px`,
      ...
      <div :key="index" v-for="(item, index) in game.obstacles" :style="{
        position: 'absolute',
        width: '20px',
        height: ' 20px',
        left: `${item.x * 20}px`,
...
.wrapper {
  ...
  display: flex;
  flex-wrap: wrap;
  width: v-bind(varWidth);
  height: v-bind(varHeight);
  margin: 0 auto;
}
</style>
